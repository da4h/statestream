  ...
  public addDependency(connector: Connector, required: boolean) {
    ...
    if (
      ...
      if (required) {
        connector.state
          .changed()
          .subscribe((_) => this.dependencyStateChanged());
      }
    }
  }
  ...
  private dependencyStateChanged() {
    if (this.state.value === ConnectionState.Available) {
      if (
        this._dependencies
          .filter((d) => d.required)
          .some((d) => d.connector.state.value === ConnectionState.Unavailable)
      ) {
    ...
    if (this.state.value === ConnectionState.Unavailable) {
      if (
        this._dependencies
          .filter((d) => d.required)
          .every((d) => d.connector.state.value === ConnectionState.Available)
      ) {
