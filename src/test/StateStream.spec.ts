...
describe('StateStream', () => {
  ...
  beforeEach(() => {
    ...
    stateStream.then(cbData);
    stateStream.onEmpty(cbEmpty);
    stateStream.onError(cbError);
    cbData.mockReset();
    cbEmpty.mockReset();
    cbError.mockReset();
  });
  ...
  describe('data', () => {
    it('first', async () => {
      ...
      expect(stateStream.state.value).toBe('data');
    });
    ...
    it('changed', async () => {
      ...
      expect(stateStream.state.value).toBe('data2');
    });
  })
